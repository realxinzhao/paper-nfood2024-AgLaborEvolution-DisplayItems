...
# SI Labor share  ----
LaborMetric <- "Labor share"

Plaborstat_AllScens %>%
  filter(grepl(LaborMetric, sector)) %>%
  mutate(scen0 = factor(scenario, levels = ScenAllLabel,
                        labels = c("Evolving", "Elasticity", "Elasticity", "Transition", "Transition",
                                   "Productivity", "Productivity", "Static") )) %>%
  mutate(HighLow = factor(scenario, levels = ScenAllLabel,
                          labels = c("Evolving", "High", "Low",  "High", "Low", "High", "Low", "Static") )) %>%
  mutate(scen0 = factor(scen0, levels = c("Evolving", "Productivity", "Transition", "Elasticity", "Static"))) ->
  df

df %>% filter(!(scenario == "Static"), region != "World") %>%
  ggplot() + facet_wrap(~region, ncol = 5, scales = "free_y") +
  geom_hline(yintercept = 0) +
  geom_line(aes(x = year, y = value, group = interaction(region, scenario), linetype = HighLow,
                color = scen0), size = 1) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  scale_color_manual(values = c("red",brewer.pal(4, "Set1")[2:4], "grey60")) +
  scale_linetype_manual(values = c(1, 4, 5, 1)) +
  labs(x = "Year", y = "Share",
       color = "Scenario", linetype = "Sensitivity") +
  theme_bw() + theme0 + theme1 + theme(legend.position = "none", axis.title.x = element_blank()) +
  ggtitle("(B) Regional agricultural labor share in labor force by scenario") -> p2

df %>% filter(!(sector == "Labor share" & scenario == "Static"), region == "World") %>%
  mutate(scen1 = "Sensitivity scenarios") %>%
  bind_rows(
    df %>% filter(scenario %in% c("Static", "Evolving"), region == "World") %>%
      mutate(scen1 = "Evolving vs. Static")
  ) %>%
  ggplot() + facet_wrap(~scen1, ncol = 5, scales = "free_y") +
  geom_line(aes(x = year, y = value, group = interaction(region, scenario), linetype = HighLow,
                color = scen0), size = 1) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  scale_color_manual(values = c("red",brewer.pal(4, "Set1")[2:4], "grey60")) +
  ...
  ggtitle("(A) World agricultural labor share in labor force by scenario") -> p1

cowplot::get_legend(p1) -> pleg
((p1 + theme(legend.position = "none")) + pleg + plot_layout(widths = c(2,0.4)) ) /p2 +
  plot_layout(heights = c(1,1.6)) -> pp

pp %>% Write_png(.name = "FigS_AgLaborShare", .DIR_MODULE = DIR_MODULE, h = 14, w = 12)

# SI Effective labor ----
LaborMetric <- "Effective labor"
Plaborstat_AllScens %>%
  filter(grepl(LaborMetric, sector)) %>%
  mutate(scen0 = factor(scenario, levels = ScenAllLabel,
                        labels = c("Evolving", "Elasticity", "Elasticity", "Transition", "Transition",
...
df %>% filter(region != "World") %>%
  ...
  scale_color_manual(values = c("red",brewer.pal(4, "Set1")[2:4], "grey60")) +
  scale_linetype_manual(values = c(1, 4, 5, 1)) +
  labs(x = "Year", y = "Index (2015 = 1)",
       color = "Scenario", linetype = "Sensitivity") +
  theme_bw() + theme0 + theme1 + theme(legend.position = "none", axis.title.x = element_blank()) +
  ggtitle("(B) Regional agricultural effective labor by scenario") -> p2
...
df %>% filter(region == "World") %>%
  ...
  theme(legend.key.size = unit(1.5, "cm"),
        legend.key.height=unit(1.5,"line")) +
  ggtitle("(A) World agricultural effective labor by scenario") -> p1

cowplot::get_legend(p1) -> pleg
((p1 + theme(legend.position = "none")) + pleg + plot_layout(widths = c(1,0.3)) ) /p2 +
  plot_layout(heights = c(1,1.6)) -> pp

pp %>% Write_png(.name = "FigS_AgLaborEffective", .DIR_MODULE = DIR_MODULE, h = 14, w = 12)
...
#   #filter(sector %in% tolower(c("Fruits", "Vegetables"))) %>%
#   Agg_reg(crop = sector, sector = element, region) %>%
#   filter(!grepl("Other use", sector)) %>%
#   Plot_Sens(#.ScenRM = "Static",
#             .Type = "R") +
#   facet_grid(crop~sector)
#   facet_wrap(~sector, scales = "fixed", nrow = 2)
#   filter(!grepl("Other use|Exp|Imp|Feed|Bio", sector))-> df
